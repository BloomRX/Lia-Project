import type { LiaAllTalkSyncResult, LiaCustomVoiceProfile } from '../../../shared/eventa'
...
import { isInside, safeFilename } from './voice-profiles'
...
/** The union `LiaAllTalkSyncResult` allows on the failure side. */
type SyncErrorCode = Extract<LiaAllTalkSyncResult, { ok: false }>['error']

function fail(error: SyncErrorCode, message: string): LiaAllTalkSyncResult {
  ...
  return {
    ...
    async ensureProfileAvailableToAllTalk(profileId) {
      ...
      try {
        ...
        return fail('fileMissing', `"${reference.filename}" is missing. It may have been moved or deleted.`)
      }
      if (sourceStat.size === 0)
        return fail('fileMissing', `"${reference.filename}" is empty.`)
      ...
      if (!filename)
        return fail('invalidExtension', `"${ext || reference.filename}" is not an audio file AllTalk can use (expected ${ALLTALK_AUDIO_EXTENSIONS.join(', ')}).`)
