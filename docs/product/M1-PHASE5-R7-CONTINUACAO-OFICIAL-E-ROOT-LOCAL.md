...
## 10. Adendo R7.2 — root cause do "segue sem o botão para instalar"

O botão não apresentava falha nenhuma: **o card inteiro não montava**. Cadeia: `manager().isInstalled()` → `resolveInstallDir` → `runtimeRootDir()` → `migrateLegacyRuntimeRootsSync` **lançava** quando o rename falhava (ERP/EBUSY — arquivo travado na árvore antiga: servidor de voz que ficou rodando da depuração manual, AV, handle do OneDrive). Como o latch era "só em sucesso", **toda** chamada re-tentava e re-lançava → o IPC `electronLiaRuntimeState` rejeitava para sempre → renderer caía em `error` → `needsInstall=false && bootstrapOutcome=false` → `RuntimeInstallCard` nunca montava. Na máquina QA a R7 é a primeira build que tenta rename de verdade (o legado Round-6 em `Roaming\proj-airi` existe), então o defeito só apareceu nela.

Fix: `adoptRuntimeRootSync(layout, deps)` em `voice-runtime-root.ts` — **adoção de root que nunca joga o app fora do card**. Migra quando pode; se o rename falhou, adota para a sessão (a) o legado mais recente **livre de caracteres proibidos** (= o root Round-6, hosts válidos da continuação oficial) — e nunca o `@proj-airi` — ou (b) o root novo **sem migrar nada** (nada copiado, nada apagado; a tentativa seguinte re-tenta no próximo start). Decisão travada por processo; log leva o motivo técnico (`runtime-root-adopted-with-fallback`, `migration-failed-fallback-*`). Wrapper Electron latches a decisão (não mais "só em sucesso"). 6 testes novos: sucesso inalterado, no-ops intocados, EPERM em rename→legacy seguro, EPERM em mkdir→mesmo destino, `@` exclusivo→root novo (nunca ressuscita o path envenenado), disco vazio→`legacy-absent` ordinário (rename nem roda). 303/303 verdes no diretório lia; a invariante migrate-propaga-EPERM **permanece** — quem absorve é a adoção, a migração continua honesta.
